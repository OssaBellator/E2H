...
The source snapshot is included in the release's generic GitHub provenance attestation and immutable GitHub Release assets. The CycloneDX runtime SBOM remains scoped to the Python wheel/sdist because it describes the package runtime dependency graph rather than the repository snapshot.

A consumer can verify and restore the source asset without trusting an autogenerated archive:
...
- `source_commit` is workflow-supplied provenance. `verify-toolchain` checks it only when the consumer supplies `--expected-source-commit`; establishing that an independently obtained commit identity is trustworthy remains the caller's responsibility.
...
- GitHub attestations provide workflow/publisher provenance. The E2H manifest, source snapshot, and verification report do not replace those attestations or claim that a dependency, runner, source archive, or workflow is intrinsically trustworthy.

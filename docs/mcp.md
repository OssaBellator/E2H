...
Handle-bound local replay is not a filesystem or credential sandbox. The child runs as the MCP service account with that account's ordinary host permissions, and the local runner inherits the service process environment before applying capsule-declared environment overrides. The configured root constrains which capsule/workspace paths the remote request can select; it does not confine the command's syscalls or absolute-path access after launch. If local replay is enabled, run the MCP server under a dedicated low-privilege account or equivalent isolated service context, minimize secrets in its environment, and treat every selectable capsule as code trusted with those permissions.

All MCP/A2A replay backends apply the same hard host-resource budget immediately after capsule revalidation and before workspace binding or Docker work. A replay is rejected if it declares more than 50 checks, would retain more than 2,000,000 aggregate stdout/stderr characters at the capsule output ceiling, or declares more than 1,800 seconds of aggregate check timeout. These service ceilings are independent of the capsule's lower per-check limits and protect the long-lived verification host from one oversized replay request.

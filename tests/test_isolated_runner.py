...
command = args[-1]
if command not in {{"read-one", "read-two"}}:
...
def test_isolated_container_replay_ignores_original_path_rebinding(
    ...
    capsule = TaskCapsule(
        ...
        sandbox=ContainerSandbox(image=IMAGE, workspace_access="read_only"),
        success=SuccessSpec(
            commands=[
                CommandCheck(id="read-one", argv=["read-one"]),
                CommandCheck(id="read-two", argv=["read-two"]),
            ]
        ),
    )
    ...
    assert result.checks[1].stdout == "inside\n"

                        ...
                        "contents": [
                            {
                                "id": "user_cli",
                                "role": "user",
                                "parts": [{"text": "Contact cli@example.com"}],
                            }
                        ],
                        "candidate_ids": ["model_cli"],
                        "response": {
                            "responseId": "response_cli",
...
def test_cli_writes_bundle_trace_and_privacy_report(tmp_path: Path) -> None:
    ...
    assert payload["provenance"]["format"] == "gemini-generate-content-json"
...
def test_cli_table_and_no_redact_review(tmp_path: Path) -> None:
    ...
    assert "gemini.json" in result.stdout
...
def test_cli_uses_custom_redaction_policy(tmp_path: Path) -> None:
    ...
    assert payload["provenance"]["redaction_policy_id"] == "gemini-cli-policy"
...
def test_cli_rejects_invalid_archive(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["ingest", "gemini-generate-content", str(source)])
